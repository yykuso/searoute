 ...
 * - 航路・港湾選択時の右側パネル表示
 * - ドロワー内コンテンツの生成と更新
 * - Wikipedia 画像の非同期読み込み
 * - 開閉アニメーションとタッチ操作のハンドリング
 */
...
export let detailDrawerContent = null;
export let detailDrawerTitle = null;
export let detailDrawerCloseBtn = null;
export let detailDrawerShareBtn = null;
...
function handleContentClick(event) {
    const actionElement = event.target.closest('[data-drawer-action]');
    if (!actionElement || !detailDrawerContent.contains(actionElement)) {
        return;
    }

    const { drawerAction, ...payload } = actionElement.dataset;
    executeDrawerAction(drawerAction, payload);
}

function handleWindowMousedown(e) {
    if (!detailDrawer.classList.contains('hidden') && !detailDrawer.contains(e.target)) {
        if (windowRef.innerWidth < 768) minimizeDetailDrawer();
    }
}
...
// --- 初期化・破棄ライフサイクル ---
export function initDetailDrawer({ documentRef = document, windowRef: windowOverride = window } = {}) {
    if (initialized) {
        return false;
    }

    detailDrawer = documentRef.getElementById('detail-drawer');
    detailDrawerContent = documentRef.getElementById('detail-drawer-content');
    detailDrawerTitle = documentRef.getElementById('detail-drawer-title');
    detailDrawerCloseBtn = documentRef.getElementById('detail-drawer-close-btn');
    detailDrawerShareBtn = documentRef.getElementById('detail-drawer-share-btn');
    sidebarGrip = documentRef.getElementById('sidebar-grip');
    windowRef = windowOverride;
    ...
    if (detailDrawerShareBtn) {
        detailDrawerShareBtn.addEventListener('click', copyShareUrl);
    }
    if (detailDrawerContent) {
        detailDrawerContent.addEventListener('click', handleContentClick);
    }

    windowRef.addEventListener('mousedown', handleWindowMousedown);
    windowRef.addEventListener('touchstart', handleWindowTouchstart, { passive: true });
    windowRef.addEventListener('resize', handleWindowResize);

    detailDrawer.addEventListener('touchstart', handleDrawerTouchStart, { passive: false });
    detailDrawer.addEventListener('touchmove', handleDrawerTouchMove, { passive: false });
    detailDrawer.addEventListener('touchend', handleDrawerTouchEnd, { passive: false });
...
export function disposeDetailDrawer() {
    if (!initialized) {
        return false;
    }

    if (detailDrawerCloseBtn) {
        detailDrawerCloseBtn.removeEventListener('click', hideDetailDrawer);
    }
    if (detailDrawerShareBtn) {
        detailDrawerShareBtn.removeEventListener('click', copyShareUrl);
    }
    if (detailDrawerContent) {
        detailDrawerContent.removeEventListener('click', handleContentClick);
    }

    if (windowRef) {
        windowRef.removeEventListener('mousedown', handleWindowMousedown);
        windowRef.removeEventListener('touchstart', handleWindowTouchstart);
        windowRef.removeEventListener('resize', handleWindowResize);
    }

    if (detailDrawer) {
        detailDrawer.removeEventListener('touchstart', handleDrawerTouchStart);
        detailDrawer.removeEventListener('touchmove', handleDrawerTouchMove);
        detailDrawer.removeEventListener('touchend', handleDrawerTouchEnd);
    }
    if (sidebarGrip) {
    ...
    detailDrawerTitle = null;
    detailDrawerCloseBtn = null;
    detailDrawerShareBtn = null;
    sidebarGrip = null;
    windowRef = null;
    initialized = false;
    return true;
